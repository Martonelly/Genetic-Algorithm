...
population_range = range(20, 60, 20)
generation_range = range(200, 800, 100)
tournament_range = range(4, 6, 1)
mutation_precentage = range(0, 10, 5)
resRound = 0
# populationLen = 20
# generationLen = 600
steadySS = False
...
for sel in range(2):
    if (sel == 0):
        ...
        for selType in range(3):
                    ...
                    for m in mutation_precentage:
                        for p in population_range:
                                for g in generation_range:
                                    ...
                                    best = population[0]
                                    results.append([p, g, solution_time, best.x, best.y, best.fitness, k, m, t])
                                    somethingCNT += 1
            ...
            headers = ["Population", "Generation", "Time (s)", "X", "Y", "Fitness", "Elitism", "Mutation chance", "Tournament Count"]
                        ...
                        for g in generation_range:
                            ...
                            for i in range(g):
                                ...
                                if (selType==0):
                                    ...
                                    offspring = tournamentSelection.select(population, round(len(population)/10), t, 0)
                                #offspring = rouletSelection.select(population,round(len(population)/10), 0)
                                #offspring = rankSelection.select(population, round(len(population)/10), 0)
                            ...
                            solution_time = time.time() - start_time                   
                            best = population[0]
                            results.append([p, g, solution_time, best.x, best.y, best.fitness, -1, m, t])
                                #resRound += 1
